...
//    var notes: [Note] = []
    ...
    fileprivate func setupFetchedResultsController() {
        ...
        fetchedResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: dataController.viewContext, sectionNameKeyPath: nil, cacheName: "notes:\(notebook.name ?? "")")
        fetchedResultsController.delegate = self
        do {
            try fetchedResultsController.performFetch()
        } catch {
            fatalError("Cannot use fetch \(error.localizedDescription)")
        }
    }
    ...
    func deleteNote(at indexPath: IndexPath) {
        guard let noteToDelete = fetchedResultsController?.object(at: indexPath) else {
            print("Cannot delete!")
            return
        }
